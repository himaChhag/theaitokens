...
export function generateStaticParams(): { slug: string }[] {
  return CATALOG.filter((m) => m.provider === "xai" && m.status === "active").map((m) => ({
    slug: m.slug,
  }));
}
...
export async function generateMetadata(
  ...
  try {
    const model = findModelByProviderAndSlug("xai", slug);
    ...
    const canonical = `/xai/${model.slug}`;
...
export default async function XaiModelPage(
  props: { params: Promise<{ slug: string }> }
) {
  const { slug } = await props.params;
  ...
  let model;
  try {
    model = findModelByProviderAndSlug("xai", slug);
  } catch {
    notFound();
  }
