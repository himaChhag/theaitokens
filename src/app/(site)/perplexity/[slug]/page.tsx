...
export function generateStaticParams(): { slug: string }[] {
  return CATALOG.filter((m) => m.provider === "perplexity" && m.status === "active").map((m) => ({
    slug: m.slug,
  }));
}
...
export async function generateMetadata(
  ...
  try {
    const model = findModelByProviderAndSlug("perplexity", slug);
    ...
    const canonical = `/perplexity/${model.slug}`;
...
export default async function PerplexityModelPage(
  props: { params: Promise<{ slug: string }> }
) {
  const { slug } = await props.params;
  ...
  let model;
  try {
    model = findModelByProviderAndSlug("perplexity", slug);
  } catch {
    notFound();
  }
