...
export function generateStaticParams(): { slug: string }[] {
  return CATALOG.filter((m) => m.provider === "meta" && m.status === "active").map((m) => ({
    slug: m.slug,
  }));
}
...
export async function generateMetadata(
  ...
  try {
    const model = findModelByProviderAndSlug("meta", slug);
    ...
    const canonical = `/meta/${model.slug}`;
...
export default async function MetaModelPage(
  props: { params: Promise<{ slug: string }> }
) {
  const { slug } = await props.params;
  ...
  let model;
  try {
    model = findModelByProviderAndSlug("meta", slug);
  } catch {
    notFound();
  }
