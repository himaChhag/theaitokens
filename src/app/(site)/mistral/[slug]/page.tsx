...
export function generateStaticParams(): { slug: string }[] {
  return CATALOG.filter((m) => m.provider === "mistral" && m.status === "active").map((m) => ({
    slug: m.slug,
  }));
}
...
export async function generateMetadata(
  ...
  try {
    const model = findModelByProviderAndSlug("mistral", slug);
    ...
    const canonical = `/mistral/${model.slug}`;
...
export default async function MistralModelPage(
  props: { params: Promise<{ slug: string }> }
) {
  const { slug } = await props.params;
  ...
  let model;
  try {
    model = findModelByProviderAndSlug("mistral", slug);
  } catch {
    notFound();
  }
