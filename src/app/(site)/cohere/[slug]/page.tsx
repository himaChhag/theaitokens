...
export function generateStaticParams(): { slug: string }[] {
  return CATALOG.filter((m) => m.provider === "cohere" && m.status === "active").map((m) => ({
    slug: m.slug,
  }));
}
...
export async function generateMetadata(
  ...
  try {
    const model = findModelByProviderAndSlug("cohere", slug);
    ...
    const canonical = `/cohere/${model.slug}`;
...
export default async function CohereModelPage(
  props: { params: Promise<{ slug: string }> }
) {
  const { slug } = await props.params;
  ...
  let model;
  try {
    model = findModelByProviderAndSlug("cohere", slug);
  } catch {
    notFound();
  }
